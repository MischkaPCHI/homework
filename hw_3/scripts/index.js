...
const createButtonHandler2 = () => {
    if(userInputText){                                                  //проверка на пустую строку
        if(userInputText !== ''){
...
const updateList = (list) =>{                                           //обновляет массив очищая всё и затем добавляет по новой всё что ему передаётся 
    ...
    listObject.append(...newTaskList);
    console.log(newTaskList);                                           //как будто ссылка на <li> в html и показавает какие классы задействованы
    console.log(todoList);                                              //просто показывает новый массив
};
